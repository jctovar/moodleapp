...
export class CoreNetworkService extends Network {
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (isOnline && hadOfflineMessage) {
                ...
                // Elimina el indicador de conexión recuperada si el dispositivo se desconecta.
                CoreHTMLClasses.toggleModeClass('core-online', false);
            }
        });
    }
    ...
    initialize(): void {
        ...
        if (CorePlatform.isMobile()) {
            // No podemos escuchar directamente onChange porque depende de onConnect y onDisconnect
            // que ya han sido sobrescritos. En su lugar, escuchamos estos eventos de forma individual.
            ...
            // Establece las constantes de Cordova en el objeto global window
            // para que la aplicación funcione correctamente en navegadores web.
    ...
    /**
     * Devuelve un observable que notifica cuando la aplicación está conectada
     * y se espera que sea una conexión estable.
     *
     * Por ejemplo, cuando se sale del modo avión, el dispositivo podría conectarse
     * primero a la red móvil y luego cambiar a WiFi. Con este observable se evita
     * hacer solicitudes de red durante estas transiciones.
     *
     * Se recomienda usar esta función en lugar de onConnect cuando se realicen
     * solicitudes de red, ya que algunos SO (ej: Android) duplican las solicitudes
     * si el tipo de conexión cambia mientras se está realizando la solicitud.
     *
     * @returns Observable.
     */
    ...
    /**
     * Dispara el observable correcto dependiendo del estado de la conexión.
     * Si está en línea, emite un evento de "conectado" inmediatamente,
     * y después de 5 segundos (cuando la conexión debería ser estable),
     * emite el evento de conexión estable.
     */
